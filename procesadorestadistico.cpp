...
{
    for(int i = 0; i < imagen->getCantidadColumnas()*imagen->getCantidadColumnas(); i++)
    {
        for(int columna = 0; columna < imagen->getCantidadColumnas(); columna++)
        {
            for(int fila = 0; fila < imagen->getCantidadFilas(); fila++)
            {
                primerosValores[i] = imagen->getPrimerComponente(columna, fila);
                segundosValores[i] = imagen->getSegundaComponente(columna, fila);
                tercerosValores[i] = imagen->getTercerComponente(columna, fila);

                valoresPromedio[i] = (imagen->getPrimerComponente(columna, fila) + imagen->getSegundaComponente(columna, fila) + imagen->getTercerComponente(columna, fila))/3.0f;
            }
...
vector<float> &ProcesadorEstadistico::getPrimerosValores()
{
    return primerosValores;
}

vector<float> &ProcesadorEstadistico::getSegundosValores()
{
    return segundosValores;
}

vector<float> &ProcesadorEstadistico::getTercerosValores()
{
    return tercerosValores;
}

vector<float> &ProcesadorEstadistico::getValoresPromedio()
{
