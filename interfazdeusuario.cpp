...
void InterfazDeUsuario::atajosDeTeclado()
{
    ...
    cout<<"Ctrl + L para limpiar la imagen. "<<endl;
...
int InterfazDeUsuario::menuInicial()
{
    system("chcp 65001");
...
int InterfazDeUsuario::opcionesDirectorio()
{
    ...
    cout<<"6) Carpeta de imágenes 5 (samples)"<<endl;
    cout<<"Ingrese 0 para cerrar el programa."<<endl;
    cin>>auxOpcion;

    while(auxOpcion != 1 and auxOpcion != 2 and auxOpcion != 3 and auxOpcion != 4 and auxOpcion!= 5 and auxOpcion != 6 and auxOpcion != 0)
    {
        cout<< "La opción ingresada es incorrecta. Vuelva a intentarlo, por favor. \a"<<endl;
        cin>>auxOpcion;
        cout<<endl;
...
void InterfazDeUsuario::opcionAtajo(string opcion)
{
    ...
    if(opcion == "L")
    {
        cout<<"Usted ha solicitado limpiar la imagen."<<endl;
    }
...
void InterfazDeUsuario::errorImagenAlterada()
{
    cout<<"No pueden llevarse acabo las operaciones solicitadas ya que la imagen esta alterada."<<endl;
}
